...
Дополнительно - https://vimeo.com/showcase/5616060
...
|git commit --amend -m "Change commit"|Скорректировать сообщение для предыдущего коммита с перезаписью данных, если они менялись|
...
|git merge *branch* --squash |Объединение двух веток с указанием имени ветки с объединением коммитов ветки в один коммит|
|git log [ --all ] --graph --oneline --decorate|История коммитов в виде графика для текущей ветки (доп. параметр для всех веток)|
|git rebase *to_branch*|Смена старта текущей ветки на конец указанной ветки|
...
|git rebase -i HEAD*number_commit*|Интерактивный режим для смены старта (можно объединить изменения)|
|git cherry-pick *commit*|Перенести выбранный коммит из другой ветки в текущую|
|git cherry-pick *branch*|Перенести последний коммит ветки в текущую ветку (*..branch* - все коммиты ветки|
|git tag *0.0.0*|Записать тег с номером версии| 
|git tag|Просмотр всех тегов| 
|git tag --list|Просмотр всех тегов| 
|git tag -d *0.0.0*|Удалить тег| 
|git stash|Выгрузка изменений во временное хранилище| 
|git stash push *file*|Выгрузка изменений во временное хранилище определенного файла в уже существующий stash| 
|git stash list|Отображение временных хранилищ| 
|git stash pop *stash*|Вернуть изменения на ветку из stash | 
|-|-| 
## Удаленно
| Команда | Описание |
|-|-|
...
|git push --tags|Отправка тега|
|git push --delete origin *0.0.0*|Удаление тега|
